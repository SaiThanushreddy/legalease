...
const MAX_INPUT_CHARS = 4000 // Reduced to save tokens
const MAX_OUTPUT_TOKENS = 512   // Reduced for efficiency
const MAX_RETRY_ATTEMPTS = 3
const BASE_RETRY_DELAY = 2000 // 2 seconds

// Updated model list - using the lightest models first to conserve quota
const MODELS_TO_TRY = [
...
function createSimplifiedRequestBody(message: string) {
  return {
    contents: [
      {
        parts: [
          {
            text: `Legal assistant. Answer briefly with disclaimer.
...
A: `,
    ...
    generationConfig: {
      ...
      topP: 0.8,
      topK: 10, // Reduced for efficiency
    },
    safetySettings: [
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requestBody = createSimplifiedRequestBody(truncatedMessage)
            ...
            if (response.status === 429) {
              ...
              if (parsedError?.error?.details) {
                ...
                if (retryInfo?.retryDelay) {
                  // Convert retry delay (e.g., "44s") to milliseconds
    ...
    if (lastError?.status === 403 || lastError?.status === 401) {
      return NextResponse.json(
        { error: "API key is invalid or doesn't have permission. Please check your Gemini API key and ensure it's enabled for the Generative Language API." },
        { status: 403 }
      )
    }
    ...
    if (lastError?.status === 429) {
      ...
      if (quotaInfo?.violations?.some((v: any) => v.quotaId?.includes("FreeTier"))) {
        errorMessage += " You're on the free tier and have hit your daily limits. Consider upgrading to a paid plan or wait for the quota to reset (usually at midnight PT)."
      } else {
        errorMessage += " Please wait before making more requests or check your billing settings."
      }
      ...
      return NextResponse.json(
        { 
          ...
          suggestions: [
            ...
            "Use shorter prompts to consume fewer tokens",
    ...
    if (lastError?.status === 404) {
      return NextResponse.json(
        { error: "None of the Gemini models are accessible with your API key. This might be a regional availability issue." },
        { status: 404 }
      )
    }
    
    return NextResponse.json(
      { error: `API request failed: ${lastError?.status || 'Unknown error'}. Please try again later.` },
